...
//  Slyyk Driver
//
//  Created by Prabin Phasikawo on 06/06/2024.
//
...
//    @Published var currentData: FBRidesModel?
    ...
    public func FBInitialize(tabRouter: TabRouter){
        Task (priority: .medium) {
            await getProfileData(tabRouter: tabRouter)
        }
    ...
    @MainActor
    private func getProfileData(tabRouter: TabRouter) {
        self.loading = true
        ...
        profileListener = documentReference.addSnapshotListener { [weak self] documentSnapshot, error in
            ...
            do {
                ...
                if self.profileData?.is_approved ?? false {
