    ...
    var body: some View {
        ...
        ScrollView {
                ...
                if VM.bookingData != nil {
                    VStack {
                        if VM.bookingData?.passenger != nil {
                            PassengerUserView(passengerData: VM.bookingData?.passenger)
                        }
                        ...
                        VStack {
                            ...
                            VStack (alignment: .leading){
                                ...
                                VStack(alignment: .leading, spacing: 10){
                                    VStack{
                                        VStack (spacing: 8){
                                            if VM.bookingData?.fare_breakdown?.count ?? 0 > 0 {
                                                ForEach(Array((VM.bookingData?.fare_breakdown?.enumerated())!), id: \.offset) { i, el in
                                                    HStack {
                                                        Text("\(el.name ?? "")")
                                                        Spacer()
                                                        if el.value != "" {
                ...
                Text("This booking does not exist")
                    .cardStyleModifier()
                    .padding()
            }
        }
...
struct PassengerUserView: View{
    var passengerData: PassengerModel?
    var body: some View{
        HStack{
            KFImage(URL(string: passengerData?.profile_picture_url ?? ""))
                .placeholder {
                    Image("default")
                        .resizable()
            ...
            VStack(alignment: .leading){
                Text(passengerData?.name ?? "")
                    ...
                    .padding(.bottom, 4.0)
                HStack {
                    if let ratingString = passengerData?.rating, let rating = Int(ratingString), rating > 0 {
