...
class PhoneVerifyVM: BaseObservableObject {
    ...
    func validatePhoneNumber() -> Bool{
        if phoneNumber.isEmpty {
            showAlert(title: "Error!", msg: "Phone Number Can't Be Empty")
            return false
        }
        else if phoneCC == 0{
            showAlert(title: "Invalid Number", msg: "Please Enter Valid Number")
            return false
        }
        return true
    }
        ...
        PhoneAuthProvider.provider().verifyPhoneNumber(phone, uiDelegate: nil) { (ID, err) in
            ...
            if err != nil {
                if err?.localizedDescription == "We have blocked all requests from this device due to unusual activity. Try again later."{
                    self.showAlert(title: "Account Blocked", msg: "Due to unusual activity please try again later")
                }
                else {
                    self.showAlert(title: "Invalid", msg: "\(err?.localizedDescription ?? "Invalid phone number")")
                }
                return
            }
            self.ID = ID!
